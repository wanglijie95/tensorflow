...
class SessionManager(object):
  ...
  * Initializing variables on startup, restoring them from the most recent
  ...
  def _restore_checkpoint(self,
    ...
    temp = time.time()
    logging.info("Checkpoint::Create session time : %.7f"%(temp - start))
    
    
    # global_step = training_util.get_global_step(graph=self._graph)
    # is_init = sess.run(state_ops.is_variable_initialized(global_step))
    # logging.info("Before Recover, global_step init : %d"%is_init)
  ...
  def server_restart_session(self, master, ps_state_op, recovery_done_op, config=None):
    """ Create a `Session`, the lastest worker is responsible for
        doing the recovery operations.

    When recovery from PS failed, each worker will call this function.
